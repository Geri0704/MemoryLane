...
class AnalyticsWorker(context: Context, workerParameters: WorkerParameters) : Worker(context, workerParameters) {
    ...
    override fun doWork(): Result {
        try {
            ...
            try {
                processWeeklyEntries()
            } catch (e: Exception) {
                println(e)
                return Result.failure(workDataOf("Failed" to "Weekly analytics process failed"))
            }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun processWeeklyEntries() {
